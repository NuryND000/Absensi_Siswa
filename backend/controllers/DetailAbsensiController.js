...
export const getDetailAbsensi = async (req, res) => {
  try {
    const detailabsensis = await DetailAbsensi.aggregate([
      {
        $lookup: {
          from: 'siswas',
          localField: 'siswa_id',
...
export const getDetailAbsensis = async (req, res) => {
  try {
    const detailabsensis = await DetailAbsensi.aggregate([
      {
        $match: {
          absensi_id: new ObjectId(req.params.id)
        }
      },
      {
        $lookup: {
...
export const saveManyDetailAbsensi = async (req, res) => {
  ...
  try {
    const inserteddetailabsensi = await DetailAbsensi.insertMany(req.body, options);
    res.status(201).json(inserteddetailabsensi);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
export const updateDetailAbsensiMany = async (req, res) => {
  try {
    ...
    if (failedUpdates.length > 0) {
      return res.status(400).json({ message: `${failedUpdates.length} updates failed` });
    }

    res.status(200).json({ message: 'All documents updated successfully', results });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
...
export const getDetailAbsensiMany = async (req, res) => {
  try {
      ...
      const absensiDetails = await Absensi.find({ absensi_id: { $in: validIds } }); // Query the database for these _id's
